...
#define IS_PLACEHOLDER_PARTIALLY_MATCH(a_szMatch) \
    ...
    else if (IS_PLACEHOLDER_PARTIALLY_MATCH("GUEST_VERSION["))
    {
        ...
        {
            *pfOutputting = false;
            return mpSetError->setErrorBoth(E_FAIL, VERR_PARSE_ERROR, mpSetError->tr("Malformed @@VBOX_COND_GUEST_VERSION[expr]@@: Missing ']' (%.*s)"),
        ...
        {
            *pfOutputting = false;
            return mpSetError->setErrorBoth(E_FAIL, VERR_PARSE_ERROR, mpSetError->tr("Malformed @@VBOX_COND_GUEST_VERSION[expr]@@: Missing expr (%.*s)"),
                                            cchPlaceholder, pachPlaceholder);
        }
        ...
        {
            *pfOutputting = false;
            return mpSetError->setErrorBoth(E_FAIL, VERR_PARSE_ERROR, mpSetError->tr("Malformed @@VBOX_COND_GUEST_VERSION[expr]@@: Only space, '>', '>=', '<', <=', and '=' are allowed at the start. (%.*s)"),
                                            cchPlaceholder, pachPlaceholder);
        }
        if (startPos >= endPos)
        {
            *pfOutputting = false;
            return mpSetError->setErrorBoth(E_FAIL, VERR_PARSE_ERROR, mpSetError->tr("Malformed @@VBOX_COND_GUEST_VERSION[expr]@@: No version string found. (%.*s)"),
                                            cchPlaceholder, pachPlaceholder);
        }
        ...
        for (size_t i = startPos; i < endPos; ++i)
        {
            ...
            {
                *pfOutputting = false;
                return mpSetError->setErrorBoth(E_FAIL, VERR_PARSE_ERROR, mpSetError->tr("Malformed @@VBOX_COND_GUEST_VERSION[expr]@@: Version string must be consist of only digits and '.', and no spaces. (%.*s)"),
                                                cchPlaceholder, pachPlaceholder);
            }
        }
